...
if ! command -v docker >/dev/null 2>&1; then
  echo "Docker not available in this devcontainer (enable docker-in-docker feature)." >&2
  exit 1
fi
if ! docker ps -a --format '{{.Names}}' | grep -q "^${NAME}$"; then
  ...
  docker exec -u www-data ${NAME} php occ app:enable ${APP_ID} || true
else
  echo "Container ${NAME} already exists. Starting..."
  docker start ${NAME} >/dev/null
fi
echo "Ensuring app enabled..."
docker exec -u www-data ${NAME} php occ app:enable ${APP_ID} >/dev/null 2>&1 || true
cat <<EOF
